...
@contextmanager
def rembg_env():
    keys = ["OMP_NUM_THREADS", "MKL_NUM_THREADS", "ORT_DISABLE_CPU_AFFINITY"]

    # 1) 先把原来的值保存起来
    old_vals = {k: os.environ.get(k) for k in keys}

    # 2) 只在 rembg 这段时间修改环境变量
    ...
    try:
        ...
        for k, v in old_vals.items():
            if v is None:
                os.environ.pop(k, None)   # 原来没这个变量，就删掉
            else:
                os.environ[k] = v         # 原来有，就改回去
...
from OrientAnything.Orient_Anything.render import render, Model
import math
axis_model = Model("./OrientAnything/Orient_Anything/assets/axis.obj", texture_filename="./OrientAnything/Orient_Anything/assets/axis.png")
